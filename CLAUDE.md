...
- ✅ BFT-Ped-SCA-Frage (HT13) **GELÖST 04-07-2026:** BFT Klemme 61 auf **IC=4 (Ped, kein Timer)** umgestellt (war IC=6). Ein Flügel öffnet und bleibt offen bis aktiver Close-Befehl — kein automatisches Schließen durch BFT. Ped-Zustand via DI1=0/DI2=0 erkennbar (live verifiziert): Voll-offen=DI1=1, Ped=DI1=0/DI2=0, Zu=DI2=1. SCA-Ausgang nicht nötig. Auto-Close für Ped übernimmt der ESP (Ch4). **v0.42:** `g_ped_aktiv`-Flag pausiert Störungs-Erkennung bei Impuls-Ped (Reset bei DI1=1 oder DI2=1).
...
**Umgesetzt (Stand 04-07-2026):** Klemme 61 = **Ped** (IC=4 Ped, **kein Timer** — geändert 04-07-2026 von IC=6) = „nur ein Flügel / Fußgänger, bleibt offen bis Close-Befehl"; Klemme 65 bleibt **Open** (IC=2, Vollöffnung). Firmware v0.37 entsprechend: Ch1/r1 = Öffnen (BFT65), Ch4/r4 = Fußgänger/Ped (BFT61). Dauerauf = gehaltenes Open auf Ch1; Ped-Dauerauf = gehaltenes Ped auf Ch4.
...
### 🚫 ESPHome-API NICHT von zwei HA-Instanzen gleichzeitig

**Befund:** Ein ESP, der per nativer API in **zwei** HA-Instanzen gleichzeitig eingebunden ist, startet fortlaufend neu.

**Herkunft:** eigene Beobachtung im Betrieb an den **Garagen-ESPs** (`busgarage-hcpbridge`, `martins-garage-hcpbridge`) — kein dedizierter Testaufbau.
